...
- Kubernetes >= 1.19
- Helm >= 3.8
- Linux/amd64 nodes for the upstream Typemill v2.26.1 image
...
The default Typemill image is pinned to the verified v2.26.1 manifest digest. When `image.digest` is set, it takes precedence over `image.tag`. To use a custom tag, clear the digest explicitly:
...
updates Typemill to v2.26.1 and adds AI timeout and reasoning-effort values without
...
replace it with the v2.26.1 digest or intentionally keep the custom pin.
